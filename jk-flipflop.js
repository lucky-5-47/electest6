// 确保JKFlipFlop类在全局作用域可用
window.JKFlipFlop = class JKFlipFlop {
    constructor() {
    ...
    initializeComponent() {
        ...
        // 添加JK触发器的SVG定义
        window.componentSvgs['jk-flipflop'] = `
            <svg class="component-svg" viewBox="0 0 200 120" xmlns="http://www.w3.org/2000/svg">
                ...
                <!-- K输入 -->
                <text x="5" y="65" font-family="Arial" font-size="12">K</text>
                ...
                <!-- 时钟输入 -->
                <text x="5" y="95" font-family="Arial" font-size="12">CLK</text>
                <line x1="25" y1="90" x2="50" y2="90" stroke="black" stroke-width="2"/>
                <path d="M45,85 L50,90 L45,95" fill="none" stroke="black" stroke-width="1"/>
                <circle class="anchor input-anchor clock-input" data-anchor-type="clock" cx="20" cy="90" r="5" fill="#0066cc"/>
    ...
    addComponentToMenu() {
        ...
        console.log('JK触发器组件菜单项已在HTML中定义');
        
        // 我们只需要确保componentSvgs中有对应的SVG定义
        if (window.componentSvgs && window.componentSvgs['jk-flipflop']) {
            console.log('JK触发器SVG定义已添加到componentSvgs');
        } else {
            console.error('无法添加JK触发器SVG定义');
        }
    }
    ...
    addComponentLogic() {
        ...
        document.addEventListener('component-created', (e) => {
            ...
            if (component.dataset.type === 'jk-flipflop') {
                this.initializeJKFlipFlop(component);
            }
        });
        ...
        document.addEventListener('signal-propagation', (e) => {
            const component = e.detail.component;
            if (component.dataset.type === 'jk-flipflop') {
                this.updateJKFlipFlopDisplay(component);
            }
        });
    }

    initializeJKFlipFlop(component) {
    ...
    updateJKFlipFlopDisplay(component) {
        const id = component.dataset.id;
    ...
    handleClockRisingEdge(clockComponent) {
        ...
        if (typeof window.anchorConnections !== 'undefined') {
            this.propagateClockToConnectedJKFlipFlops(clockAnchor);
        }
    }

    propagateClockToConnectedJKFlipFlops(clockAnchor) {
        // 递归查找连接到该时钟的所有JK触发器
        const visited = new Set();
        this.findConnectedJKFlipFlops(clockAnchor, visited);
    }

    findConnectedJKFlipFlops(anchor, visited) {
        ...
        for (const connectedAnchor of connectedAnchors) {
            ...
            // 如果是JK触发器且连接的是时钟输入
            if (component.dataset.type === 'jk-flipflop' && connectedAnchor.classList.contains('clock-input')) {
                this.updateJKFlipFlopOnClockEdge(component);
            }

            // 递归查找其他连接
            this.findConnectedJKFlipFlops(connectedAnchor, visited);
        }
    }

    updateJKFlipFlopOnClockEdge(component) {
        const id = component.dataset.id;
        const state = window.sequentialElements.get(id);
        if (!state) return;
        ...
        const jInputAnchor = inputAnchors[0];
        const kInputAnchor = inputAnchors[1];

        // 获取J和K输入值
        const jInputValue = this.getInputSignalValue(jInputAnchor);
        const kInputValue = this.getInputSignalValue(kInputAnchor);
        ...
        // J=1, K=0: 置位(Q=1)
        // J=1, K=1: 翻转状态
        if (jInputValue === 0 && kInputValue === 0) {
            // 保持状态不变
        } else if (jInputValue === 0 && kInputValue === 1) {
            // 复位
            state.q = 0;
            state.qBar = 1;
        } else if (jInputValue === 1 && kInputValue === 0) {
            // 置位
            state.q = 1;
            state.qBar = 0;
        } else if (jInputValue === 1 && kInputValue === 1) {
            // 翻转状态
        ...
        // 更新状态
        state.lastJ = jInputValue;
        state.lastK = kInputValue;
...
document.addEventListener('DOMContentLoaded', function() {
    // 初始化JK触发器
    const jkFlipFlop = new JKFlipFlop();
    console.log('JK触发器组件已初始化');
});
